...
idx <- 10 * (1:as.integer(nrow(aero)/10)) - as.integer(runif(1,min=0, max=10))

# not included in modeling
temp <- aero[-idx,]

# save not included and remove
saveRDS(temp, "Input/excluded_measurements10.rds")
rm(temp)
...
saveRDS(aero,"Input/aero_cluster10.rds")
saveRDS(aero.grid, "Input/grid_cluster10.rds")
saveRDS(SCALE_rain, "Input/SCALE_rain10.rds")
