...
idx <- 15 * (1:as.integer(nrow(aero)/15)) - as.integer(runif(1,min=0, max=15))

# not included in modeling
temp <- aero[-idx,]

# save not included and remove
saveRDS(temp, "Input/excluded_measurements.rds")
rm(temp)
...
saveRDS(aero,"Input/aero_cluster.rds")
saveRDS(aero.grid, "Input/grid_cluster.rds")
saveRDS(SCALE_rain, "Input/SCALE_rain.rds")
